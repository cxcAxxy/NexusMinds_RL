...
class Grasp_single_object(Task):
    ...
    def compute_reward(self) -> torch.Tensor:
        ...
        d_mid_sq = (d_mid ** 2).sum(-1)  # ||d_mid||^2

        r_neg = torch.exp(-self.alpha_mid* d_mid_sq)  # exp(-α_neg * d_neg_min)


        # 抵消前期的影响，表示的是离地的高度,设置必须为0.2
        reward_grasp = self.c4* (self.c5 * (0.2 - goal_distance)+self.c6 * r_neg)
        ...
        total_reward =self.c1 * reward_grasp +self.c2 * reward_pos
        return total_reward